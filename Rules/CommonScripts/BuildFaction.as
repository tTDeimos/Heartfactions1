		...
		if (b.buildOnGround)
		{
			...
			if (!fail)
			{
				...
				if (!fail && b.name == "factionbase")
				{
					...
					Vec2f[] stop;
        			if(getMap().getMarkers("rpg_stop", stop )) 
        			{
            			Vec2f v = stop[0];
            			if(pos.y <= v.y)
            			{
        			...
        			f32 dist = 99999999.0f;
        			CBlob@[] bases;
        			getBlobsByName("factionbase", @bases);
        			for(int i=0; i<bases.length;i++)
        			{
        				CBlob@ temp = bases[i];
        				if(temp !is null)
        				{
        					f32 dist_ = (temp.getPosition() - this.getPosition()).Length();
        					if(dist_ < dist)
        						dist = dist_;
        				}
        			}
        			if(dist < 300.0f)//if there is another base that is closer than about 50 blocks
        				fail = true;
				}
			}
