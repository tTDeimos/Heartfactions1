...
void onTick(CBlob@ this)
{
	         ...
	         (pressed_a2 || (specialShieldState)))
	{
		...
		if (XORRandom(8) == 0)
		{
			if(isServer())
			{
				server_CreateBlob("glassvial",-1,this.getPosition());
			}
		}
		else
		{
			if (XORRandom(11) == 0)
			{
				if(isServer())
				{
					server_CreateBlob("necriumvial",-1,this.getPosition());
				}
			}
			else
			{
				if (XORRandom(8) == 0)
				{
					if(isServer())
					{
						server_CreateBlob("flamevial",-1,this.getPosition());
					}
				}
				else
				{
					if (XORRandom(5) == 0)
					{
						if(isServer())
						{
							server_CreateBlob("healingvial",-1,this.getPosition());
						}
					}
					else
					{
						if (XORRandom(6) == 0)
						{
							if(isServer())
							{
								server_CreateBlob("lifevial",-1,this.getPosition());
							}
						}
						else
						{
							if (XORRandom(5) == 0)
							{
								if(isServer())
								{
									server_CreateBlob("lightvial",-1,this.getPosition());
								}
							}
							else
							{
								if (XORRandom(4) == 0)
								{
									if(isServer())
									{
										server_CreateBlob("minevial",-1,this.getPosition());
									}
								}
								else
								{
									if (XORRandom(2) == 0)
									{
										if(isServer())
										{
											server_CreateBlob("bombvial",-1,this.getPosition());
										}
									}
									else
									{
										if (XORRandom(2) == 0)
										{
											if(isServer())
											{
												server_CreateBlob("gunpowdervial",-1,this.getPosition());
											}
										}
										else
										{
											if (XORRandom(6) == 0)
											{
												if(isServer())
												{
													server_CreateBlob("kegvial",-1,this.getPosition());
												}
											}
											else if (XORRandom(5) == 0)
											{
												if(isServer())
												{
													server_CreateBlob("firefountainvial",-1,this.getPosition());
												}
											}
											else
											{
												if (XORRandom(8) == 0)
												{
													if(isServer())
													{
														server_CreateBlob("chestrainvial",-1,this.getPosition());
													}
												}
												else
												{
													if (XORRandom(11) == 0)
													{
														if(isServer())
														{
															server_CreateBlob("cataclysmvial",-1,this.getPosition());
														}
													}
													else
													{
														if (XORRandom(3) == 0)
														{
															if(isServer())
															{
																server_CreateBlob("bombrainvial",-1,this.getPosition());
															}
														}
														else
														{
															if (XORRandom(2) == 0)
															{
																if(isServer())
																{
																	server_CreateBlob("inkblotvial",-1,this.getPosition());
																}
															}
															else
															{
																if (XORRandom(2) == 0)
																{
																	if(isServer())
																	{
																		server_CreateBlob("coinvial",-1,this.getPosition());
																	}
																}
																else
																{
																	if(isServer())
																	{
																		server_CreateBlob("kegrainvial",-1,this.getPosition());
																	}
																}
															}
														}
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		         ...
		         knight.state <= KnightStates::sword_cut_down) // cut state
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
			{
				...
				DoAttack(this, 0.75f, attackAngle, attackarc, Hitters::sword, delta, knight);
			}
			else if (delta >= 9)
			{
				knight.swordTimer = 0;
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			{
				DoAttack(this, 0.75f, -(vec.Angle()), 120.0f, Hitters::sword, delta, knight);
			}
