// AcolyteOfTheSun logic

#include "ThrowCommon.as"
#include "AcolyteOfTheSunCommon.as";
...
void onTick(CBlob@ this)
{
	...
	int slash_charge = KnightVars::slash_charge;
	int slash_charge_level2 = KnightVars::slash_charge_level2;
	int slash_charge_limit = KnightVars::slash_charge_limit;
	if(this.exists("speedmult"))
	{
		float speedmult = this.get_f32("speedmult");
		slash_charge *= speedmult;
		slash_charge_level2 *= speedmult;
		slash_charge_limit = slash_charge + slash_charge_level2 + (10.0f * speedmult);
	}
	         ...
	         (pressed_a2 || (specialShieldState)))
	{
		...
		CBlob@ b = server_CreateBlob("keg",-1,this.getPosition()+Vec2f((this.isFacingLeft() ? -8 : 8),-8));
		if(b !is null)
		{
			Vec2f vel((this.isFacingLeft() ? -1.0f : 1.0f), -0.01f);
			b.setVelocity(vel * 1200.0);
	...
	{
		CBlob@ wall = server_CreateBlob("bigfirewall",-1,this.getPosition()+Vec2f((this.isFacingLeft() ? -5 : 5),-5));
		if(wall !is null)
		{
			Vec2f vel((this.isFacingLeft() ? -1.0f : 1.0f), -1.5f);
			wall.setVelocity(vel * 2.0);
			wall.server_SetTimeToDie(20.0f);
		}
		SetKnocked(this, 32);
	}
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, KnightInfo@ info)
{
	...
	damage = damage * this.get_f32("dmgmult");
	if (aimangle < 0.0f)
	{
		aimangle += 360.0f;
	}
	...
	{
		attack_distance *= this.get_f32("rangemult");
	}
...
void shootBeam(CBlob@ this, float Vel)
{
	if(getNet().isServer())
	{
		if(this.exists("misc"))
		{
			...
			if(beamname != "no") //So we don't have to check for the ring names twice.
			{
				Vec2f pos = this.getPosition();
				Vec2f aimPos = this.getAimPos();
				CBlob@ beam = server_CreateBlob(beamname, -1, pos);
				if(beam !is null)
				{
					beam.SetDamageOwnerPlayer(this.getPlayer());
					Vec2f shootpoint = aimPos - pos;
					shootpoint.Normalize();
					beam.setVelocity((shootpoint * 3) * Vel);
					shootpoint = shootpoint;
					f32 angle = (beam.getVelocity()).Angle();
					beam.setAngleDegrees(-angle + 90);
					beam.server_setTeamNum(this.getTeamNum());
				}
			}
		}
	}
