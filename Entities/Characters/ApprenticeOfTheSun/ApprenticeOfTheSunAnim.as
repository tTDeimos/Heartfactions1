// ApprenticeOfTheSun animations

#include "ApprenticeOfTheSunCommon.as";
...
{
	addRunnerTextures(this, "apprenticeofthesun", "ApprenticeOfTheSun");
...
{
	ensureCorrectRunnerTexture(this, "apprenticeofthesun", "ApprenticeOfTheSun");
}
...
void onTick(CSprite@ this)
{
	...
	if(blob.exists("speedmult"))
	{
		float speedmult = blob.get_f32("speedmult");
	...
	{
		if (knight.swordTimer < slash_charge)
		{
			this.SetAnimation("draw_sword");
		}
		else if (knight.swordTimer < slash_charge_level2)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 0;
		}
		else if (knight.swordTimer < slash_charge_limit)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 1;
			shinydot = true;
	...
	if (shiny !is null)
	{
		...
		if (shinydot)
		{
			f32 range = (slash_charge_limit - slash_charge_level2);
			f32 count = (knight.swordTimer - slash_charge_level2);
