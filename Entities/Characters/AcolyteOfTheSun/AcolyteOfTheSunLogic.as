...
void onTick(CBlob@ this)
{
	         ...
	         (pressed_a2 || (specialShieldState)))
	{
		...
		knight.swordTimer = 0;
		//FIRE WALL
		CBlob@ wall = server_CreateBlob("firewall",-1,this.getPosition()+Vec2f((this.isFacingLeft() ? -1 : 1),-0.5));
		SetKnocked(this, 80);
		if(wall !is null)
		{
			Vec2f vel((this.isFacingLeft() ? -1.0f : 1.0f), -1.5f);
			wall.setVelocity(vel * 1.2);
		}
	}
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		CBlob@ projectile = server_CreateBlob("fireorb",-1,this.getPosition()+Vec2f((this.isFacingLeft() ? -8 : 8),-4));
		if(projectile !is null)
		{
			Vec2f vel((this.isFacingLeft() ? -7.0f : 7.0f), -2.5f);
