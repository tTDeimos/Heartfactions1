...
const s32 hit_frame = 5;
const f32 hit_damage = 0.15f;
...
bool RecdHitCommand(CBlob@ this, CBitStream@ params)
{
		...
		if(map !is null)
		{
			if(map.getSectorAtPosition(tilepos, "no build") is null)
			{
				...
				if (getNet().isServer())
				{
					...
					{
						damage *= this.get_f32("dmgmult");
					}
					float hitnum;
					hitnum = this.get_u8("dmgresidue") / 100.0f; //So we have previous damage residue
					int resdmg = damage + hitnum;
					for(hitnum += damage; 1 <= hitnum; hitnum--)
					{
						map.server_DestroyTile(tilepos, 1.0f, this);
					}// So hitnum should now have whatever residue is left over from both this hits and previous hits - seems so \o/
					this.set_u8("dmgresidue", hitnum * 100.0f);
					print("res damage " + resdmg);
					Material::fromTile(this, type, resdmg);
				}
	...
	else
	{
		...
		if(blob !is null)
		{
			...
			if (getNet().isServer())
			{
				attack_power*= this.get_f32("dmgmult");
...
void Pickaxe(CBlob@ this)
{
	...
	Animation@ anim = sprite.getAnimation("strike");
	if(this.exists("speedmult"))
	{
		anim.time = 2 * Maths::Max(this.get_f32("speedmult"), 0.5f);
	}
	if(!strikeAnim)
	{
		this.get("hitdata", @hitdata);
...
bool canHit(CBlob@ this, CBlob@ b, Vec2f tpos, bool extra = true)
{
	...
	if(b.hasTag("invincible") || b.hasTag("slaveproof") || b.hasTag("workshop"))
	{
		return false;
	}
