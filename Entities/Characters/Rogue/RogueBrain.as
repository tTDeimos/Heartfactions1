...
void onTick(CBrain@ this)
{
	...
	if (target !is null)
	{
		...
		{
			if (!visibleTarget || distance > 180.0f)
			{
				strategy = Strategy::chasing;
			}
		}
...
void AttackBlob(CBlob@ blob, CBlob @target)
{
	...
	{
		// start attack
		if (XORRandom(Maths::Max(3, 30 - (difficulty + 8) * 2)) == 0 && (getGameTime() - blob.get_u32("attack time")) > 10)
		{
	...
	else if (targetDistance < 55.0f && getGameTime() - blob.get_u32("attack time") < (Maths::Min(13, difficulty + 8))) // release and attack when appropriate
	{
